    ...
    do
    {
        ...
        if(Coordenadas.x>1050&&Coordenadas.x<1300 &&Coordenadas.y>270&&Coordenadas.y<340)
        {
            ...
            if(resp==13)
            {
                ...
                {
                    if(Usuario_ativo(Byte)==1)
                    {
                      if(Aba_Consulta(4,Byte)==1)
                        {
                            Compra_Usuario(Byte);
                        }
                    }
...
int Cadastro_produto()
{
    ...
    else
	{
	    ...
	    }while(!(isdigit(prdt_int[0])&&isdigit(prdt_int[1])&&
                   isdigit(prdt_int[2])&&isdigit(prdt_fl)&&isdigit(prdt_fl2)));
	  int intr[5];
      for(i=0;i<3;i++)
      {
            intr[i]=prdt_int[i]-48;
      }
       int f1=prdt_fl-48;
       int f2=prdt_fl2-48;

        produto.preco=intr[0]*100+intr[1]*10+intr[2]+f1*0.1+f2*0.01;
	...
	do{
		do
		{
			...
			for(i=0;i<4;i++)
            {
      	    ...
      	    }while(!(isdigit(dt_int[0])&&isdigit(dt_int[1])&&
                   isdigit(dt_int[2])&&isdigit(dt_int[3])));
	      ...
	      produto.ano_lancamento=intr[0]*1000+intr[1]*100+intr[2]*10+intr[3];
	        int ano=Ano();
	        int valor=produto.ano_lancamento;
	      if(valor>=1950 && valor<=ano)
		  {
		  	 saida=1;
		  }
              ...
              else if(y==12)
            {
					do{
						do
						{
					      ...
					      int valor=intr[0]*1000+intr[1]*100+intr[2]*10+intr[3];
					        int ano=Ano();

					      if(valor>=1950 && valor<=ano)
						  {
						  	 saida=false;
						  	 produto.ano_lancamento=valor;
						  	 break;
						  }
...
int Usuario_ativo(fpos_t Byte)
{
    ...
    if(usuario.ativo!=1)
    {
        MessageBox(0,"USUÁRIO DESATIVADO","ATENÇÃO",0);
...
void redefine(void){
    system("color 3F");
    system("MODE con cols=123 lines=28");
    system("title STEAM - by: LGMT");
    {
       ...
       if(!achou)
       {
               ...
               for(i=1;i<diferenca;i++)
               {
                   ...
                   fwrite(&novo,sizeof(novo),1,p);
                    ...
                    novo.mes=relatorio.mes+diferenca;
                   novo.lucro=novo.entrada=0;
                   novo.saida=0;
                   novo.dia=0;
                   fwrite(&novo,sizeof(novo),1,p);
...
int main()
{
    ...
    if(p==NULL)
    {
        if(MessageBox(0,"\t\t               BEM VINDO AO PROGRAMA!\n\nAntes de começar a utiliza-lo é necessario que leia atentamente e aceite os termos abaixo.\n"
                      "\n1-Para utilizar o programa é necessário que o cmd esteja configurado com a fonte consolas tamanho 24 e esteja em janela maximizada posicionada no topo esquerdo da tela;"
"\n\n2-Para navegar no menu principal faz-se necessário o uso do mouse e a tecla <ENTER> para escolher a opção desejada, no menu de edição é utilizado também as setas direcionais do teclado;"
"\n\n3-Ao aceitar os termos de uso será necessário o cadastro para prosseguir com a utilização;"
		"\n\t   Ao aceitar, você concorda com todos os termos acima.","TERMOS DE USO",4)==6)
        {
            Aba_Cadastro(1);
            p = fopen("Usuario", "wb");
            main();
